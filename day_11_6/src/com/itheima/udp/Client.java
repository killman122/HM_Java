...
public class Client {// 这里是一个客户端
    public static void main(String[] args) throws IOException {
        ...
        DatagramPacket datagramPacket = new DatagramPacket(bytes, 0, bytes.length, InetAddress.getLocalHost(), 666);//参数一buffer封装要发送出去的数据(和数据的流式存储一样,都是使用byte字节类型数据存储),参数二是开始的消息的索引值,参数三length数据长度(发送出去的数据大小字节个数),参数四服务端的ip地址(找到服务端主机),参数五服务端程序端口号(此处为客户端代码不是服务端)
